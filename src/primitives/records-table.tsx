...
/* Column headings are display-only; the internal column keys below stay fixed so
   the property popover and sorting keep working. */
...
const DEFAULT_RECORDS_LABELS: RecordsTableLabels = {
  name: "Company",
  tags: "Categories",
...
export default function RecordsTable({ rows = INITIAL_ROWS, fill = false, labels }: { rows?: RecordRow[]; fill?: boolean; labels?: Partial<RecordsTableLabels>; variant?: string }) {
  const l = { ...DEFAULT_RECORDS_LABELS, ...labels };
        ...
        <table ref={tableRef} className="records-table" style={{ width: columnWidthsLocked ? tableWidth : "100%", minWidth: tableWidth }}>
          ...
          <thead>
            <tr>
              ...
              <HeaderCell label={l.tags} selected={prop?.col === "Categories"} onPick={openProp("Categories")} sort={sort} onSort={toggleSort} onResizeStart={startColumnResize("categories")} resizing={resizingColumn === "categories"} icon={<Icon size={15}>{TYPE_GLYPHS["Multi select"]}</Icon>} />
              <HeaderCell label={l.last} selected={prop?.col === "Last interaction"} onPick={openProp("Last interaction")} sortKey="last" sort={sort} onSort={toggleSort} onResizeStart={startColumnResize("last")} resizing={resizingColumn === "last"} icon={<Icon size={15}>{TYPE_GLYPHS.Date}</Icon>} />
              <HeaderCell label={l.strength} selected={prop?.col === "Connection strength"} onPick={openProp("Connection strength")} sortKey="strength" sort={sort} onSort={toggleSort} onResizeStart={startColumnResize("strength")} resizing={resizingColumn === "strength"} icon={<Icon size={15}>{TYPE_GLYPHS["Single select"]}</Icon>} />
              <HeaderCell label={l.links} selected={prop?.col === "Links"} onPick={openProp("Links")} sort={sort} onSort={toggleSort} onResizeStart={startColumnResize("links")} resizing={resizingColumn === "links"} icon={<Icon size={15}>{TYPE_GLYPHS.URL}</Icon>} />
              {aiAdded && (
